...
function testSqlite() {
    ...
    var insert = function (db) {
        var file = require('fs').readFileSync('./test.png');
        db.serialize(function () {
            return __awaiter(this, void 0, void 0, function () {
                var stmt, rowNumber, i;
                return __generator(this, function (_a) {
                    db.run('PRAGMA cipher_compatibility = 4');
                    // To open a database created with SQLCipher 3.x, use this:
                    // db.run("PRAGMA cipher_compatibility = 3");
                    db.run("PRAGMA key = 'mysecret'");
                    db.run('CREATE TABLE IF NOT EXISTS lorem (myid INTEGER PRIMARY KEY, info TEXT, myimage BLOB)');
                    stmt = db.prepare('INSERT INTO lorem(myid, info, myimage) VALUES (NULL, $info, $myimage)');
                    rowNumber = 10000;
                    console.log("Inserting ".concat(rowNumber, " rows....."));
                    for (i = 0; i < rowNumber; i++) {
                        // await wait();
                        stmt.run({
                            $info: "Lorem ".concat(Math.random()),
